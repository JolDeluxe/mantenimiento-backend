...
const estadosIncluidos = new Set<EstadoFalla>([
  EstadoFalla.ABIERTA,
  EstadoFalla.REHABILITADA,
  EstadoFalla.CERRADA,
]);
...
function razonExclusionFalla(falla: {
  ...
  if (!falla.confirmadoPorId) return "SIN_CONFIRMADOR";
  if (!estadosIncluidos.has(falla.estado)) return "ESTADO_NO_INCLUIDO";
  if (!calidadesIncluidas.has(falla.calidadDato)) return `${falla.calidadDato}_EXCLUIDO`;
  return null;
}
...
export class BIDetailService {
  ...
  static async obtenerDetalleMaquina(input: MachineDetailInput) {
    const { maquinaId, desdeStr, hastaStr, paginaEventos, limiteEventos, ahora } = input;
    ...
    const fallaAnterior = await prisma.fallaMaquina.findFirst({
      where: {
        ...
        calidadDato: { in: Array.from(calidadesIncluidas) },
    ...
    const eventosFallas: BIDetailFallaData[] = fallas.map((f) => ({
      ...
      duracionMinutos: f.fechaRestauracion && f.fechaFallaConfirmada
        ? Math.max(0, (f.fechaRestauracion.getTime() - f.fechaFallaConfirmada.getTime()) / 60000)
        : null,
      excluidoMTTR: f.estado === EstadoFalla.ABIERTA || !f.fechaRestauracion,
      razonExclusion: razonExclusionFalla(f) ?? (!f.fechaRestauracion ? "FALLA_ABIERTA_MTTR" : null),
    }));
