// list/list_actividades.ts
// Listado de actividades sin máquina: fuerza scope=actividades (maquinaId = null).
...
export const listarActividades = async (req: Request, res: Response) => {
  try {
    ...
    const query: TicketFilterQuery = { ...req.query as unknown as TicketFilterQuery, scope: "actividades" };
    ...
    await registrarError("LIST_ACTIVIDADES", req.user?.id || null, error);
    return res.status(500).json({ error: "Error interno al obtener actividades" });
  }
};
