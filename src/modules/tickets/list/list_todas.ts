// list/list_todas.ts
// Handler explícito para scope=general del módulo HOY/ACTIVOS.
// Decisión: no reutilizar list_bandeja.ts para TODAS; bandeja queda para Bandeja General.
...
import {
  ...
  ordenarTicketsOperativamente
} from "../helper";

export const listarTodas = async (req: Request, res: Response) => {
  try {
    const user = req.user!;
    const query: TicketFilterQuery = { ...req.query as unknown as TicketFilterQuery, scope: "general" };

    const { page, limit, sort, estado } = query;
    const offset = (page - 1) * limit;
    const usaOrdenOperativo = Boolean(query.vista);
    ...
    await registrarError("LIST_TODAS", req.user?.id || null, error);
    return res.status(500).json({ error: "Error interno al obtener tareas activas" });
  }
};
