...
export const validarProteccionDepartamento = (
  ...
  if (departamentoActual.nombre === env.SYS_DEPTO_CRITICO) {
    ...
    if (datosNuevos.nombre && datosNuevos.nombre.trim() !== env.SYS_DEPTO_CRITICO) {
      throw new Error(`El departamento '${env.SYS_DEPTO_CRITICO}' es crítico para el sistema y no puede ser renombrado.`);
    }

    if (datosNuevos.estado === Estatus.INACTIVO) {
      throw new Error(`El departamento '${env.SYS_DEPTO_CRITICO}' es fundamental para el sistema y no puede ser desactivado.`);
    }
  }
};